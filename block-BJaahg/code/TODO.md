...
```js
var one = addOne(0);//addOne is not defined
const addOne = (num) => {
  return num + 1;
};
var two = addOne(1);
...
isAwesome();//awesome is not defined
```
...
console.log(name);//name is not not defined
```
...
sample();//Arya Stark 'first' John Snow 'second'
```
...
if (true) {
  ...
  myFunc();//username is not defined
}
//
```
